...
export function makeMailItem( _data: MailGet.Output  ,index:number ,onPress?: (event: GestureResponderEvent) => void) {
    return ( 
                ...
                <Grid style={{marginTop:10,marginBottom:20,marginLeft:15,marginRight:15}}  >
                    <Row size={1} style={iStyle.bottomBorder} >
                        <Col size={5} >
                            {_data.status==ReceiveStatus.unreceived.toString() &&
                                <Text style={[ {color:'#F3BA1F'}, {marginTop:10},styles.titleFontSize ,]}>{_data.date.split("T")[0]} {_("m_record")}</Text>   
                            }
                            {_data.status==ReceiveStatus.received.toString() &&
                                <Text style={[  {marginTop:10},styles.titleFontSize ,]}>{_data.date.split("T")[0]} {_("m_norecord")}</Text>   
                            }                        
                        </Col>
                        <Col size={3}  >
                    ...
                    <Row size={2} >
                        ...
                        <Col style={styles.topMargin} >
                            ...
                            <Row size={1} >
                                <Text style={[styles.modalcontent,styles.contentfontSize]}>{_("m_sender")}:{_data.sender}</Text>   
                            </Row>
                            <Row size={1} >
                                <Text style={[styles.modalcontent,styles.contentfontSize]}>{_("m_re")}:{_data.receiver}</Text>   
                            </Row>
